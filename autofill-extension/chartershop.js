(() => {
  const { passengers, setValue, setDropdown, createButton } =
  ...
  function fillChartershop(data) {
    ...
    pax.forEach((p, idx) => {
      ...
      const genderVal = /MS|F/.test(gender) ? "1" : "0";
      setDropdown(form.querySelector(".passenger-sex"), genderVal);
      setValue(
        form.querySelector(".passenger-lname"),
        p.last_name || p.lastName || ""
      );
      setValue(
        form.querySelector(".passenger-fname"),
      ...
      const number =
        p.ps_number || p.passport_number || p.passportNumber || p.number || "";
